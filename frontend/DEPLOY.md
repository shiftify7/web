...
| `RESEND_FROM` | `Shiftify <leads@your-domain.tld>` | Verified Resend sender |
| `RESEND_TO` | optional | Resend-only fallback recipient; `LEAD_EMAIL_TO` is preferred |

Resend is the primary provider. When `BACKEND_URL` is configured, the Node
backend tries Resend first and Gmail second. Configure Gmail only on the Node
backend — Cloudflare Pages Functions cannot open Gmail SMTP connections.

For direct Pages Function mode, configure `LEAD_EMAIL_TO`, `RESEND_API_KEY`,
and `RESEND_FROM`. Do not add Hostinger/SMTP variables; they are not used.
